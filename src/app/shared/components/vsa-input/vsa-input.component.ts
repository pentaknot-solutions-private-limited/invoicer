import { Component, Input, Output, EventEmitter, Self, Optional, OnChanges, AfterViewInit } from "@angular/core";
...
export class VSAInputComponent implements ControlValueAccessor, OnChanges, AfterViewInit {
  ...
  constructor(@Self() @Optional() public control: NgControl) {
    ...
    this.errorMessages.set('minlength', () => `The No. of characters should not be less than 0.`);
    this.errorMessages.set('custom', () => '' + this.config?.attributes.errorMessage + '');
  ...
  ngAfterViewInit() {
    ...
    this.errorMessages.set('required', () => `This Field is required.`);
    this.errorMessages.set('minlength', () => `The No. of characters should not be less than 0.`);
    this.errorMessages.set('pattern', () => '' + this.config?.attributes.customPatternMessage + '');
    if (this.control && this.control.control && this.isIntervalSet == false) {
      this.isIntervalSet = true;
      setInterval(() => {
        this.control.control && this.control.control.patchValue(this.dataModel[this.config.fieldKey]);
      }, 100);
    }
  }
  ...
  ngOnChanges() {
    if(this.control)
    if (this.config?.attributes.pattern && this.config.attributes.isMandatory) {
